...
from app.schemas.services import AuthTokenInfo, UserData
...
def test_get_user_data_from_jwt_valid(context, secret):
    ...
    user_data = UserData(sub=uuid4(), scope=ScopeType.ACCESS)
    jwt_token = jwt.encode(payload=user_data.model_dump(mode="json"), key=jwt_secret, algorithm="HS256")
    service = AuthUtils()
    expected_user_data = service._get_user_data_from_jwt(jwt_token, "HS256", secret)
    assert expected_user_data == user_data


@pytest.mark.parametrize("secret", [None, "secret"])
def test_get_user_data_from_jwt_valid_headers(context, secret):
    jwt_secret = secret or "fake-secret"
    token_schema = "Bearer"
    alg = "HS256"
    user_data = UserData(sub=uuid4(), scope=ScopeType.ACCESS)
    jwt_token = jwt.encode(payload=user_data.model_dump(mode="json"), key=jwt_secret, algorithm="HS256")
    service = AuthUtils()
    auth_schema = AuthTokenInfo(token_type=token_schema, public_key=jwt_secret, alg=alg)
    expected_user_data = service.get_user_data_from_header_string(
        auth_header=f"{token_schema} {jwt_token}", auth_info=auth_schema
    )
    assert expected_user_data == user_data
...
def test_get_user_data_from_jwt_invalid(context, token, secret, exception):
    ...
    with pytest.raises(BaseServiceError) as exc:
        service._get_user_data_from_jwt(token, "HS256", secret)
        assert isinstance(exc, BaseServiceError)


async def test_get_user_data_invalid_token_type(context):
    service = AuthUtils()
    token = "Fake " + jwt.encode(payload={"key": "value"}, key="secret", algorithm="HS256")
    with pytest.raises(BaseServiceError):
        service.get_user_data_from_header_string(
            token, AuthTokenInfo(alg="HS256", token_type="Bearer", public_key="secret")
        )
...
async def test_get_user_data_valid_jwt(context, secret):
    ...
    _user_data = service._get_user_data_from_jwt(token, "HS256", secret)
...
async def test_get_user_data_invalid_alg(context):
    ...
    token = jwt.encode(payload=user_data.model_dump(mode="json"), key="secret", algorithm="HS256")
    with pytest.raises(BaseServiceError):
        service._get_user_data_from_jwt(token, "RS256", secret)
