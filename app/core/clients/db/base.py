...
from sqlalchemy import Executable, Result, TextClause, text
...
class SQLAlchemyAsyncDbBaseClient(ABC):
    ...
    async def execute_stmt(
        ...
        scalar = False if isinstance(stmt, (str, TextClause)) else scalar
        ...
        try:
            ...
            proxy_result = proxy_result.unique() if unique else proxy_result
            if need_result:
                return self._collect_result_from_proxy(proxy_result, need_result, scalar, mapped)
    ...
    async def _try_execute_stmt(
        ...
        raise DatabaseException(self.DB_ERROR_MESSAGE.format(exc=exception)) from exception
